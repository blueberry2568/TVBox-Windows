...
public static class UrlUtil
{
    ...
    public static string Convert(string url)
    {
        ...
        {
            "assets" => url.Replace(prefix, FongMi.TV.Server.LocalServer.Instance.GetAddress("/")),
            "proxy" => url.Replace(prefix, FongMi.TV.Server.LocalServer.Instance.GetAddress("/proxy?")),
            "file" => FongMi.TV.Server.LocalServer.Instance.GetAddress("/file/") + Uri.EscapeDataString(url[prefix.Length..]).Replace("%2F", "/").Replace("%5C", "/"),
            _ => url,
        };
    }
